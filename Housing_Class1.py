...
class HouseType:
    def __init__(self, type , HouseNum, cznum,size1, size2, vintage,devices, is_new = False):
        ...
        self.is_new = is_new  #True or False
    ...
    def HHemissions_heatcool(self,year):
        ...
        for k in range(0,num):           
            ...
            if self.devices[k].name == "Cond":
                ...
                print "III",self.type, i, self.devices[k].HDD, self.devices[k].CDD  ,emisheat, emiscool,self.devices[k].AvgRefLeaks(year) 
...
def getDead(p1_homes,devices, homeType, fuelName, k, yr):
   # print "dead test", device.name, device.fuel.name
    for dev in devices:
       print "testtest", yr, dev.C,k, len(devices), dev.name,fuelName, dev.annualreplacement(yr)
...
def getAppDead(p1_homes,devices, homeType, fuelName, AppName, k, yr):
    ...
    if "HP" in AppName:
        return sum([dev.annualreplacement(yr) for dev in devices if (dev.C ==k  and dev.fuel.name== fuelName and "HP" in dev.name )]) 
    elif "Cooler" in AppName:
        return sum([dev.annualreplacement(yr) for dev in devices if (dev.C ==k  and dev.fuel.name== fuelName and "Cooler" in dev.name )]) 
    elif "Cond" in AppName:   
         return sum([dev.annualreplacement(yr) for dev in devices if (dev.C ==k  and dev.fuel.name== fuelName  and "Cond" in dev.name)]) 
...
def LearningRateCost(stats, deviceType,  Cum, year, LR = LR_HP):    #Assuming this is used only for HPConditioners..
    ...
    if deviceType == "Cond" :
        ...
        ctr = 1
        devcost = HPCapex1 
        while (cum > 2 * HPCumulativeFloor * ctr and devcost >= HPCapexFloor ):   #HPCumulativeFloor is defined in Inputs_Appliances_Retrofit
...
print "SF1", k, year, round(heng1,2), round(ceng1,2), round(heng11,2),round(ceng12,2)   #, "..", round(hhemis1,3), round(hhemis2,3),round(hhemis_refrig,3) ,round(hhheatemis,3), round(hhcoolemis,3)
#print "\n"
