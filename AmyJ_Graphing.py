...
from AmyJ_TotalEnergy import *


## Graphing residential energy use by end-use

def graphYear(categories, yeardays):
    """
    ...
    for i in range(1, len(yeardays) + 1):
        ...
        if i == 12:
            hrAvg = monthAvg(categories, yeardays[i-1], 366)
            graphMonth(categories, hrAvg, i)
            
        else: 
            hrAvg = monthAvg(categories, yeardays[i-1], yeardays[i])
            graphMonth(categories, hrAvg,i)
...
def graphMonth(categories, categoryHrAvg, month):
    ...
    plt.legend()
    plt.savefig('plots1/' + 'CZ13_HC_PVSize_'+str(pv_size) + '(kW).png')
...
def graphingMonth(categories, month, yeardays):
 ...
 # print "testTEST", i, len(yeardays)
    ...
    plt.savefig('plots1/' + "month_"+ str(month) + 'PVSize_'+ str(pv_size)+ 'kW_CZ13_HC_April16.png')


graphYear(categories, year)
